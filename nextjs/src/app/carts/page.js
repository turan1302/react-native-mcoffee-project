...
import ListItem from "@/components/carts/listItem";
...
const CartPage = (props) => {
    ...
    useEffect(() => {
        if (search !== "") {
            searchCarts(page, search);
        } else {
            getCarts(page);
        }
    }, [page]);

    const handleCarts = (page) => {
        setPage(page);
    }
    ...
    const getCarts = async (data = "") => {
        ...
        await RestClient.getRequest(AppUrl.carts + `?page=${page}`, {
            headers: {
    ...
    const searchCarts = async (data = "", search = "") => {
        setIsLoading(true);
        await props.AuthStore.getToken();
        const token = (props.AuthStore.appState !== null) ? props.AuthStore.appState.user.access_token : null;
        const page = (data === "") ? 1 : data.page;

        await RestClient.getRequest(AppUrl.carts_search + `?search=${search}&page=${page}`, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
    ...
    const removeItem = (id) => {
        Notification.delete().then((result) => {
            if (result.isConfirmed) {
                let newItems = items.data?.filter(item => item.c_id !== id);
    ...
    const removeItemAPI = async (id) => {
        ...
        await RestClient.postRequest(AppUrl.carts_delete + `/${id}`, {}, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
        ...
        isLoading ? (
            ...
            <>
                <div className="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 className="h3 mb-0 text-gray-800">Sepet</h1>
                </div>
                            ...
                            {(items.data?.length >= 1) &&
                                <div className={"mt-3"}>
                                    <Pagination prevButtonText={"Önceki"} nextButtonText={"Sonraki"}
                                                changePage={handleCarts} data={items}/>
                                </div>
                            }
...
export default inject("AuthStore")(observer(CartPage));
