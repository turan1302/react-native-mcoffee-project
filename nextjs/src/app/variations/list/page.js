...
import ListItem from "@/components/variations/listItem";
...
const VariationsListPage = (props) => {
    ...
    useEffect(() => {
        if (search !== "") {
            searchVariations(page, search);
        } else {
            getVariations(page);
        }
    }, [page]);

    const handleVariations = (page) => {
        setPage(page);
    }

    const getVariations = async (data = "") => {
        setIsLoading(true);
        ...
        await RestClient.getRequest(AppUrl.variations + `?page=${page}`, {
            headers: {
    ...
    const searchVariations = async (data = "", search = "") => {
        setIsLoading(true);
        await props.AuthStore.getToken();
        const token = (props.AuthStore.appState !== null) ? props.AuthStore.appState.user.access_token : null;
        const page = (data === "") ? 1 : data.page;

        await RestClient.getRequest(AppUrl.variations_search + `?search=${search}&page=${page}`, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
    ...
    const changeStatus = (id) => {
        let newItems = items.data.map((item, index) => {
            return (item.vy_id === id) ? {...item, vy_status: !item.vy_status} : {...item}
        });
    ...
    const changeStatusAPI = async (id) => {
        ...
        await RestClient.postRequest(AppUrl.variations_status + `/${id}`, {}, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
    ...
    const removeItem = (id) => {
        Notification.delete().then((result) => {
            if (result.isConfirmed) {
                let newItems = items.data?.filter(item => item.vy_id !== id);
    ...
    const removeItemAPI = async (id) => {
        ...
        await RestClient.postRequest(AppUrl.variations_delete + `/${id}`, {}, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
        ...
        isLoading ? (
            ...
            <>
                <div className="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 className="h3 mb-0 text-gray-800">Varyasyonlar</h1>
                            ...
                            <div className="row">
                                ...
                                <div className="col-lg-1">
                                    <button className="btn btn-info btn-md" type={"button"}
                                            onClick={() => searchVariations("", search)}><i
                            ...
                            {(items.data?.length >= 1) &&
                                <div className={"mt-3"}>
                                    <Pagination prevButtonText={"Önceki"} nextButtonText={"Sonraki"}
                                                changePage={handleVariations} data={items}/>
                                </div>
                            }
...
export default inject("AuthStore")(observer(VariationsListPage));
