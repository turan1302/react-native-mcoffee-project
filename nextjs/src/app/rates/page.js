...
import ListItem from "@/components/rates/listItem";
...
const RatePage = (props) => {
    ...
    useEffect(() => {
        if (search !== "") {
            searchRates(page, search);
        } else {
            getRates(page);
        }
    }, [page]);

    const handleRates = (page) => {
        setPage(page);
    }
    ...
    const getRates = async (data = "") => {
        ...
        await RestClient.getRequest(AppUrl.rates + `?page=${page}`, {
            headers: {
    ...
    const searchRates = async (data = "", search = "") => {
        setIsLoading(true);
        await props.AuthStore.getToken();
        const token = (props.AuthStore.appState !== null) ? props.AuthStore.appState.user.access_token : null;
        const page = (data === "") ? 1 : data.page;

        await RestClient.getRequest(AppUrl.rates_search + `?search=${search}&page=${page}`, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
    ...
    const removeItem = (id) => {
        Notification.delete().then((result) => {
            if (result.isConfirmed) {
                let newItems = items.data?.filter(item => item.rt_id !== id);
    ...
    const removeItemAPI = async (id) => {
        ...
        await RestClient.postRequest(AppUrl.rates_delete + `/${id}`, {}, {
            headers: {
                "Authorization": "Bearer " + token,
            }
        }).then((res) => {
        ...
        isLoading ? (
            ...
            <>
                <div className="d-sm-flex align-items-center justify-content-between mb-4">
                    <h1 className="h3 mb-0 text-gray-800">Kahve Oylamaları</h1>
                </div>
                            ...
                            {(items.data?.length >= 1) &&
                                <div className={"mt-3"}>
                                    <Pagination prevButtonText={"Önceki"} nextButtonText={"Sonraki"}
                                                changePage={handleRates} data={items}/>
                                </div>
                            }
...
export default inject("AuthStore")(observer(RatePage));
