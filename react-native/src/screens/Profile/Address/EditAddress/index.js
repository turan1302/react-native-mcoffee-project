...
import EditCustomHeader from "../../../../components/Profile/Address/EditCustomHeader";
...
const EditAddress = (props) => {
  ...
  useEffect(() => {
    let focusHandler = navigation.addListener("focus", () => {
      getAddressData(route.params.add_id);
    });
  ...
  const getAddressData = async (id) => {
    ...
    await RestClient.getRequest(AppUrl.address_edit + `/${route.params.add_id}`, {
      headers: {
      ...
      if (status === 200) {
        ...
        setCorporateForm(result.data.address.add_type);
      }
  ...
  const removeAddress = async (id) => {
    ...
    await RestClient.getRequest(AppUrl.address_delete + `/${id}`, {
      headers: {
        "Authorization": "Bearer " + token,
      },
    }).then((res) => {
  ...
  const _handleSubmit = async (values, { setSubmitting }) => {
    ...
    await RestClient.postRequest(AppUrl.address_update + `/${route.params.add_id}`, values, {
      headers: {
        "Authorization": "Bearer " + token,
      },
    }).then((res) => {
  ...
  return (
    <AuthLayout>
      <View style={styles.container}>
        <EditCustomHeader id={route.params.add_id} removeAddress={removeAddress} title={"Adres Güncelle"} />
        <ScrollView showsVerticalScrollIndicator={false}>
          <Formik enableReinitialize={true} initialValues={{
            add_city: address.add_city,
            add_district: address.add_district,
            add_title: address.add_title,
            add_desc: address.add_desc,
            add_tax_no: address.add_tax_no,
            add_tax_office: address.add_tax_office,
            ...
            add_tax_no: corporateForm === 1 ? Yup.string().required("Vergi Numarası alanı gereklidir") : Yup.string().nullable(),
            add_tax_office: corporateForm === 1 ? Yup.string().required("Vergi Dairesi alanı gereklidir") : Yup.string().nullable(),
            ...
            {({ values, errors, touched, handleChange, handleBlur, handleSubmit, isValid, isSubmitting }) => (
              <View style={styles.form_area}>
                ...
                {(touched.add_district && errors.add_district) &&
                  <Text style={styles.error_text}>{errors.add_district}</Text>}
                ...
                {(corporateForm === 1) && (
                  <>
                    ...
                    {(touched.add_tax_office && errors.add_tax_office) &&
                      <Text style={styles.error_text}>{errors.add_tax_office}</Text>}

                    <View style={styles.tax_no_area}>
                      <TextInput value={values.add_tax_no} onChangeText={handleChange("add_tax_no")}
                                 onBlur={handleBlur("add_tax_no")} keyboardType={"number-pad"}
                                 placeholderTextColor={"#000"}
                                 placeholder={"Vergi Numarası"} style={styles.tax_no_input} />
                    </View>
                    {(touched.add_tax_no && errors.add_tax_no) &&
                      <Text style={styles.error_text}>{errors.add_tax_no}</Text>}
                  </>
                )}

                <TouchableOpacity onPress={handleSubmit} disabled={(!isValid || isSubmitting)}
                                  style={styles.save_button_area}>
                  <Text style={styles.save_button_text}>Adres Güncelle</Text>
                </TouchableOpacity>
              </View>
            )}
          </Formik>
...
export default inject("AuthStore")(observer(EditAddress));
